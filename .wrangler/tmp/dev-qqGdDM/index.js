...
// .wrangler/tmp/bundle-nGIexu/checked-fetch.js
...
// .wrangler/tmp/bundle-nGIexu/strip-cf-connecting-ip-header.js
...
async function requireAdminAuth(request, env) {
  ...
  return new Response(JSON.stringify({
    ...
    methods: {
      ...
      "header": "Provide X-Admin-API-Key header or Authorization: Bearer token"
    }
  }), {
    status: 401,
    headers: { "Content-Type": "application/json" }
...
var getCorsHeaders = /* @__PURE__ */ __name((env) => {
  const isDevelopment = env.ENVIRONMENT === "development";
  const allowedOrigin = isDevelopment ? "*" : "https://component-management.vercel.app";
  return {
...
adminRouter.options("*", (request, env) => {
  const corsHeaders2 = getCorsHeaders(env);
...
adminRouter.get("/stats", async (_, env) => {
  const corsHeaders2 = getCorsHeaders(env);
  try {
...
function sanitizeHTML(html) {
  ...
  sanitized = sanitized.replace(/style\s*=\s*["'][^"']*["']/gi, (match) => {
    let styleContent = match.replace(/style\s*=\s*["']([^"']*)["']/i, "$1");
...
function validateAndSanitizeComponent(data) {
  ...
  const sanitized = {
    ...
    tags: validated.tags.map((tag) => tag.trim()).filter(Boolean)
  };
  return {
    success: true,
    data: sanitized
...
// src/index.ts
var router = e();
var getCorsHeaders2 = /* @__PURE__ */ __name((env) => {
  const isProduction = env.ENVIRONMENT === "production";
  return {
    "Access-Control-Allow-Origin": isProduction ? "https://component-management.vercel.app" : "*",
    // ローカル開発では全てのオリジンを許可
    "Access-Control-Allow-Methods": "GET, POST, PUT, DELETE, OPTIONS",
    "Access-Control-Allow-Headers": "Content-Type, Authorization, X-Requested-With",
    "Access-Control-Allow-Credentials": "true",
...
router.options("*", (request, env) => {
  return new Response(null, { headers: getCorsHeaders2(env) });
});
router.get("/api/health", (request, env) => {
  const corsHeaders2 = getCorsHeaders2(env);
  return new Response(JSON.stringify({
    status: "ok",
...
// .wrangler/tmp/bundle-nGIexu/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-nGIexu/middleware-loader.entry.ts
