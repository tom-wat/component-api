...
// .wrangler/tmp/bundle-p4Wi6S/checked-fetch.js
...
// .wrangler/tmp/bundle-p4Wi6S/strip-cf-connecting-ip-header.js
...
var JWTManager = class _JWTManager {
  ...
  async createJWT(payload, expirySeconds) {
  ...
  async verifyToken(token) {
    ...
    const signature = Uint8Array.from(atob(encodedSignature.replace(/-/g, "+").replace(/_/g, "/")), (c2) => c2.charCodeAt(0));
    const isValid2 = await crypto.subtle.verify("HMAC", key, signature, encoder.encode(message));
    if (!isValid2) {
  ...
  static fromEnv(env) {
    ...
    return new _JWTManager(env.JWT_SECRET, config);
  }
};
...
async function validateJWT(token, env, request) {
  if (!env.JWT_SECRET) {
...
__name(checkCookieAuth, "checkCookieAuth");
...
function validateReferer(request, env) {
  ...
  if (referer) {
    return allowedOrigins.some(
      (allowedOrigin) => referer.startsWith(allowedOrigin)
    );
  }
  return false;
}
__name(validateReferer, "validateReferer");
function validateJWTSignature(request) {
  const authHeader = request.headers.get("Authorization");
  if (authHeader?.startsWith("Bearer ")) {
    return true;
  }
...
__name(validateJWTSignature, "validateJWTSignature");
function validateCustomHeaders(request) {
  ...
  if (contentType?.includes("application/json")) {
    ...
    const hasCustomHeaders = request.headers.get("X-Requested-With") || request.headers.get("X-App-Name") || request.headers.get("Authorization");
...
__name(validateCustomHeaders, "validateCustomHeaders");
function requireCSRFProtection(request, env) {
  const method = request.method.toUpperCase();
...
__name(requireCSRFProtection, "requireCSRFProtection");
async function requireAdminAuthWithRateLimit(request, env) {
...
__name(requireAdminAuthWithRateLimit, "requireAdminAuthWithRateLimit");

// src/admin-routes.ts
var adminRouter = e({ base: "/api/admin" });
var getCorsHeaders = /* @__PURE__ */ __name((env, request) => {
  const isDevelopment = env.ENVIRONMENT === "development";
  let allowedOrigin;
  if (request) {
    const origin = request.headers.get("Origin");
    ...
    if (isDevelopment && origin && (origin.includes("localhost") || origin.includes("127.0.0.1"))) {
      ...
      allowedOrigin = "https://component-management.vercel.app";
...
adminRouter.all("*", async (request, env) => {
  if (request.method === "OPTIONS") {
    return;
  }
  const corsHeaders2 = getCorsHeaders(env, request);
...
adminRouter.options("*", (request, env) => {
  const corsHeaders2 = getCorsHeaders(env, request);
  console.log("OPTIONS handler called with corsHeaders:", corsHeaders2);
  return new Response(null, { headers: corsHeaders2 });
});
adminRouter.get("/deleted-components", async (request, env) => {
  const corsHeaders2 = getCorsHeaders(env, request);
  try {
...
adminRouter.post("/components/:id/restore", async (request, env) => {
  const corsHeaders2 = getCorsHeaders(env, request);
  try {
...
adminRouter.delete("/components/:id/purge", async (request, env) => {
  const corsHeaders2 = getCorsHeaders(env, request);
  try {
    const { id } = request.params;
    if (!id) {
...
adminRouter.delete("/purge-old", async (request, env) => {
  const corsHeaders2 = getCorsHeaders(env, request);
  try {
    const url = new URL(request.url);
...
adminRouter.get("/stats", async (request, env) => {
  const corsHeaders2 = getCorsHeaders(env, request);
  try {
...
adminRouter.get("/all-components", async (request, env) => {
  const corsHeaders2 = getCorsHeaders(env, request);
  try {
    const url = new URL(request.url);
    const limit = Math.min(parseInt(url.searchParams.get("limit") || "50"), 100);
...
authRouter.post("/login", async (request, env) => {
  try {
    const csrfResult = requireCSRFProtection(request, env);
    if (csrfResult) {
      const responseBody2 = await csrfResult.text();
      return new Response(responseBody2, {
        status: csrfResult.status,
...
authRouter.post("/logout", async (request, env) => {
  try {
    ...
    const cookieHeader = request.headers.get("Cookie");
    const sessionId = getCookieValue2(cookieHeader, "adminSession");
...
authRouter.get("/status", async (request, env) => {
  try {
    const cookieHeader = request.headers.get("Cookie");
    const sessionId = getCookieValue2(cookieHeader, "adminSession");
    if (!sessionId || !env.SESSIONS) {
...
var ComponentSchema = external_exports.object({
  name: external_exports.string().min(1, "Component name is required").max(100, "Component name too long"),
  category: external_exports.string().max(50, "Category name too long").default("Other"),
  html: external_exports.string().max(5e4, "HTML content too large").default(""),
  css: external_exports.string().max(5e4, "CSS content too large").default(""),
  js: external_exports.string().max(5e4, "JavaScript content too large").default(""),
  tags: external_exports.union([
...
var UpdateComponentSchema = ComponentSchema.partial().refine(
...
var dangerousHTMLPatterns = [
  ...
  /&gt;.*?&lt;\/script&gt;/gi,
  /&#x[0-9a-f]+;?/gi,
  // 16進エンティティエンコーディング回避試行
  /&#[0-9]+;?/gi
  // 10進エンティティエンコーディング回避試行
];
var dangerousCSSPatterns = [
  /javascript:/gi,
  /expression\s*\(/gi,
  /behavior\s*:/gi,
  /binding\s*:/gi,
...
__name(encodeHTMLEntities, "encodeHTMLEntities");
function sanitizeHTMLTags(html) {
...
__name(sanitizeHTMLTags, "sanitizeHTMLTags");
function sanitizeHTMLAttributes(tag) {
...
__name(sanitizeHTMLAttributes, "sanitizeHTMLAttributes");
function sanitizeHTML(html) {
  ...
  sanitized = sanitized.replace(/&lt;/gi, "<").replace(/&gt;/gi, ">").replace(/&quot;/gi, '"').replace(/&#x27;/gi, "'").replace(/&#x2F;/gi, "/");
  sanitized = sanitizeHTMLTags(sanitized);
  dangerousHTMLPatterns.forEach((pattern) => {
    sanitized = sanitized.replace(pattern, "");
  });
  sanitized = sanitized.replace(/<(\w+)(\s[^>]*)?\s*><\/\1>/gi, "").replace(/\s+/g, " ").trim();
  return sanitized;
}
...
function sanitizeCSS(css) {
  ...
  sanitized = sanitized.replace(/url\s*\(\s*["']?([^"')]+)["']?\s*\)/gi, (match, url) => {
    ...
    const isUrlSafe = safeSchemes.some(
      (scheme) => url.toLowerCase().startsWith(scheme) || url.startsWith("/")
      // 相対パス
    );
...
__name(sanitizeCSS, "sanitizeCSS");
function sanitizeJS(js) {
  ...
  sanitized = sanitized.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/.*$/gm, "");
  ...
  const allowedPatterns = [
    /^[\s\w\-._]+$/m,
    // Simple identifiers and properties
    /getElementById\(['"][^'"]*['"]\)/gi,
    /querySelector\(['"][^'"]*['"]\)/gi,
...
__name(sanitizeJS, "sanitizeJS");
function validateAndSanitizeComponent(data) {
  ...
  const sanitized = {
    ...validated,
    name: validated.name.trim().replace(/[<>]/g, ""),
    // Remove angle brackets from name
    ...
    tags: validated.tags.map((tag) => tag.trim().replace(/[<>]/g, "")).filter(Boolean)
  };
  const warnings = [];
...
__name(validateAndSanitizeComponent, "validateAndSanitizeComponent");
function validateAndSanitizeUpdate(data) {
  ...
  if (validated.category !== void 0) {
    sanitized.category = validated.category.trim();
  }
  if (validated.html !== void 0) {
    sanitized.html = sanitizeHTML(validated.html);
  }
  if (validated.css !== void 0) {
    sanitized.css = sanitizeCSS(validated.css);
  }
  if (validated.js !== void 0) {
    sanitized.js = sanitizeJS(validated.js);
  }
  if (validated.tags !== void 0) {
    sanitized.tags = validated.tags.map((tag) => tag.trim()).filter(Boolean);
  }
  return {
    success: true,
    data: sanitized
...
__name(validateAndSanitizeUpdate, "validateAndSanitizeUpdate");

// src/index.ts
var router = e();
var getSecurityHeaders = /* @__PURE__ */ __name((env) => {
  const isDevelopment = env.ENVIRONMENT === "development";
  return {
...
var getCorsHeaders2 = /* @__PURE__ */ __name((env, request) => {
  const isDevelopment = env.ENVIRONMENT === "development";
  let allowedOrigin;
  if (request) {
    const origin = request.headers.get("Origin");
...
var getAllHeaders = /* @__PURE__ */ __name((env, request) => {
  return {
...
router.get("/api/components", async (request, env) => {
  const headers = getAllHeaders(env, request);
  try {
    const url = new URL(request.url);
...
router.get("/api/components/:id", async (request, env) => {
  const headers = getAllHeaders(env, request);
  try {
    const { id } = request.params;
    if (!id) {
...
router.post("/api/components", async (request, env) => {
  const headers = getAllHeaders(env, request);
  try {
    const csrfResult = requireCSRFProtection(request, env);
    if (csrfResult) {
...
router.put("/api/components/:id", async (request, env) => {
  const headers = getAllHeaders(env, request);
  try {
    const csrfResult = requireCSRFProtection(request, env);
    if (csrfResult) {
...
router.delete("/api/components/:id", async (request, env) => {
  const headers = getAllHeaders(env, request);
  try {
    const csrfResult = requireCSRFProtection(request, env);
    if (csrfResult) {
...
// .wrangler/tmp/bundle-p4Wi6S/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-p4Wi6S/middleware-loader.entry.ts
